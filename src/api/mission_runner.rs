...
pub async fn run_opencode_turn(
    ...
    // The permit is released during tool execution so other missions can use the LLM,
    // and re-acquired (best-effort) when tools finish.
    let provider_semaphore = provider_hint
        .as_deref()
        .and_then(|p| PROVIDER_SEMAPHORES.get(p));
    let mut provider_permit = if let Some(semaphore) = provider_semaphore {
    ...
    loop {
        tokio::select! {
            ...
            // Release/re-acquire provider permit based on tool execution state.
            // While tools run, the LLM is idle — another mission can use the slot.
            changed = sse_tool_depth_rx.changed() => {
                if changed.is_ok() {
                    let depth = *sse_tool_depth_rx.borrow();
                    if depth > 0 && provider_permit.is_some() {
                        provider_permit = None;
                        tracing::info!(
                            mission_id = %mission_id,
                            tool_depth = depth,
                            "Released provider permit during tool execution"
                        );
                    } else if depth == 0 && provider_permit.is_none() {
                        if let Some(semaphore) = provider_semaphore {
                            match semaphore.try_acquire() {
                                Ok(permit) => {
                                    provider_permit = Some(permit);
                                    tracing::info!(
                                        mission_id = %mission_id,
                                        "Re-acquired provider permit after tool completion"
                                    );
                                }
                                Err(_) => {
                                    tracing::debug!(
                                        mission_id = %mission_id,
                                        "Provider permit held by another mission; continuing without"
                                    );
                                }
                            }
                        }
